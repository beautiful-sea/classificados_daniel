      ...
      <div class="table-responsive">
        <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
          <table class="table datatable-project dataTable no-footer dtr-column"
            ...
            <tr v-if="avaliacoes.data.length" v-for="avaliacao in avaliacoes.data" role="row" class="odd">
              ...
              <td>
                <div class="btn-group" role="group" aria-label="Basic example">
                  <button @click="setAvaliacaoModal(avaliacao)" type="button" class="btn btn-primary "  data-toggle="modal" data-target="#showReviewModal" >
                    Ver
                  </button>
                  <button type="button" class="btn btn-danger "  @click="deletarAvaliacao(avaliacao)">
                    Excluir
                  </button>
                </div>
              </td>
            ...
            <tr v-if="!avaliacoes.data.length" role="row" class="odd">
              <td class="dtr-control" style="display: none;"></td>
...
export default {
  ...
  methods: {
    ...
    getAvaliacoes() {
      let self = this;
       axios.get('/anunciantes/avaliacoes' )
          .then(response => {
            self.avaliacoes = response.data
          })
          .catch(error => {
            console.log(error)
          })
    ...
    deletarAvaliacao(avaliacao) {
      ...
      swal.fire( {
        ...
        if ( result.isConfirmed ) {
          axios.delete('/anunciantes/avaliacoes/'+avaliacao.id )
              .then(response => {
                //toastr de sucesso
                swal.fire( {
                  title: 'Excluído!',
