<template>
  <div>
    ...
    <div class="d-flex justify-content-between">
      <button v-if="!editandoAgenda" class="btn btn-info btn-sm" @click="editandoAgenda = !editandoAgenda">
        Adicionar datas disponíveis
      </button>
      <div v-else>
        <button class="btn btn-danger btn-sm" @click="editandoAgenda = !editandoAgenda">
          Cancelar
        </button>
    ...
    <div class="row">
      ...
      <div v-if="editandoAgenda">
        ...
        <select class="form-control" multiple v-model="horarios">
          <option v-for="horario in horariosPossiveis" :value="horario">{{ horario }}</option>
        </select>
      </div>

      <div class="mt-3" v-if="!editandoAgenda && diaSelecionado">
        <h3>[{{(diaSelecionado.date.getDate())+"/"+(diaSelecionado.date.getMonth()+1)+"/"+diaSelecionado.date.getFullYear()}}] Horários disponíveis <button  @click="adicionarHorario()" class=" btn btn-rounded btn-sm btn-info"><i class="fa  fa-plus-circle"></i></button>

        </h3>
        <div  v-for="(hds,index) in horariosDiaSelecionado" class="d-flex ">
          <input :disabled="hds.highlight !== 'green'" type="time" class="form-control"   v-model="horariosDiaSelecionado[index].horario">
          <button class="btn  btn-danger" v-if="hds.highlight === 'green'" @click="removerHorario(hds.horario)">
            <i class="fas fa-trash"></i>
          </button>
          <button class="btn btn-info" v-if="hds.highlight !== 'green'" @click="verAgendamento(hds)">
            <i class="fas fa-list"></i>
          </button>
        </div>
        <button class="btn btn-success" @click="salvarHorariosDoDiaSelecionado">
          Salvar
        </button>
      </div>
...
export default {
  ...
  methods: {
      ...
      axios.get(url)
          .then(response => {
            ...
            let htmlText = '' +
                ...
                '<p class="'+response.data.status_label_bootstrap+'">'+response.data.status_label+'</p>' +
                '</div>' +
                '</div>';
    ...
    salvarHorariosDoDiaSelecionado() {
      ...
      let horariosDiaSelecionadoFormatado = this.horariosDiaSelecionado.map((data) => {
    ...
    visualizarData(day) {
      ...
      let attributes = this.attributes.filter((data) => {
        ...
        dataAttributo = dataAttributo.getFullYear() + '-' + (dataAttributo.getMonth() + 1) + '-' + dataAttributo.getDate();
        let dataDay = new Date(day.date);
        dataDay = dataDay.getFullYear() + '-' + (dataDay.getMonth() + 1) + '-' + dataDay.getDate();
    ...
    salvarDatas() {
      ...
      let datas = this.attributes.filter((data) => {
        return data.id === undefined;
      }).map((data) => {
        return data.dates;
      });
      this.loading = true;
      axios.post(url, {datas: datas,horarios: this.horarios})
          .then(response => {
            this.loading = false;
